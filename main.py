...
def load_all_pdfs_from_docs() -> None:
    ...
    print(
        f"Successfully loaded {len(loaded_files)} PDF file(s), total content length: {len(cv_content)} characters"
    )
...
def _chat_with_me_impl(message: str, cv_path: Optional[str] = None) -> str:
    ...
    if cv_path:
        try:
            ...
            return json.dumps(
                {"status": "error", "message": f"Failed to load CV: {str(e)}"}, indent=2
            )
...
Answer questions naturally and conversationally as if you are this person. Don't be too professional, act like a normal person with fun personality."""
        ...
        return json.dumps(
            {"status": "error", "message": f"Error calling OpenAI API: {str(e)}"},
            indent=2,
        )
...
def chat_with_me(message: str, cv_path: Optional[str] = None) -> str:
    """Chat with the Puneet S. Bagga's digital twin based on a sample CV (Jake's template)."""
